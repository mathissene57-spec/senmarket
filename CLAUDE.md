...
1. **This Next.js repo** — a Next.js 14 (App Router) app. Beyond Supabase-backed authentication (login/register/magic link/password reset) and a landing page, it now has a real **consumer catalogue/cart** (`app/catalogue`, `app/produit/[id]`, `app/panier`) and a real **vendor dashboard** (`app/dashboard`), and it does call business tables and RPCs directly — see "Consumer catalogue/cart UI" and "Vendor dashboard UI" below. (An earlier version of this file said the opposite — that there was no such UI and zero business-table/RPC calls anywhere in the repo. That's no longer true; don't assume it without checking the current tree.)
...
- **App Router, no `src/` dir**: routes live under `app/` (`app/page.tsx` landing page, `app/login/page.tsx` auth page, `app/catalogue`, `app/produit/[id]`, `app/panier`, `app/dashboard`). `@/*` path alias maps to the repo root (`tsconfig.json`).
- **Supabase auth has three separate client entry points**, matching the `@supabase/ssr` pattern — keep using the right one for the context:
  ...
  - `lib/supabase/server.ts` — server client (`createServerClient`, async, reads `next/headers` cookies), for Server Components/Actions. Also the client used by `lib/supabase/catalogue.ts` and `lib/supabase/dashboard.ts` for their business-table reads (both anon-key, RLS-gated — see below).
...
- **Route protection**: `middleware.ts` redirects unauthenticated requests to `/login` for any path under `/dashboard`. `app/dashboard/page.tsx` exists and additionally does its own connected-vendor check server-side (see below) — the middleware redirect and that in-page check are two separate layers, not one.
...
### Consumer catalogue/cart UI

- **`lib/supabase/catalogue.ts`** — read-only data layer for the catalogue, used by `app/catalogue/page.tsx` (`getCatalogue()`) and `app/produit/[id]/page.tsx` (`getProduit(id)`). Queries `boutiques`, `produits`, `produit_images`, `promotions` directly (anon key, so subject to the RLS behavior documented under "The real Supabase backend" below). Two deliberate design points baked into this file, don't "fix" them without re-reading the comments in the source: `produits.boutique_id` has no FK to `boutiques.id` in the real schema, so the code does two separate queries instead of one PostgREST embed; and because RLS does **not** hide an active product whose own boutique is inactive, both `getCatalogue()` and `getProduit()` filter that out in application code.
- **Promotions**: `chargerPromotionsActives()` in the same file loads active promotions and picks the highest-`priorite` one per product, checking the `[date_debut, date_fin]` window in JS rather than in SQL (mirrors the `senmarket-html` prototype's logic). `app/produit/[id]/page.tsx` deliberately passes the **full, non-promo price** to `AjouterAuPanier` — see the comment in that file: `creer_commande_complete` recomputes price from `produits.prix` server-side and never looks at `promotions`, so showing a discounted price there would promise a discount the order won't actually honor.
- **Cart (`lib/panier/PanierProvider.tsx`)** — client-only React context, persisted to `localStorage` (`senmarket_panier`), no backend calls until checkout. Enforces **single-boutique carts**: adding a product from a different boutique than what's already in the cart returns a conflict instead of mixing, and `app/produit/[id]/AjouterAuPanier.tsx` surfaces that as a "clear cart and add this item instead" prompt.
- **Checkout (`app/panier/page.tsx`)** — "Passer commande" calls `supabase.rpc('creer_commande_complete', ...)` **directly from the client** with only `produit_id`/`quantite` per line (never price — see RPC description below); on success it clears the cart and offers a prefilled WhatsApp deep link (`wa.me/<boutique_whatsapp>?text=...`) to the boutique, using the boutique's WhatsApp number as stored on `boutiques`, not a SenMarket-owned channel.

### Vendor dashboard UI

- **`lib/supabase/dashboard.ts`** — `getDashboard()` reads the connected vendor's own boutiques/produits/commandes (anon key, but RLS + the `vendor_id = auth.uid()` ownership model does the real gating here, unlike the public catalogue reads above). Also wraps `stats_clients_boutique`/`top_produits_boutique`, treating a failed call as "no stats available" rather than crashing the page (kept even after the Phase 3B.4 fix documented below, as a general defensive-handling pattern, not a workaround for that one bug).
- **`app/dashboard/page.tsx`** — server component; if there's no session it shows a "please log in" message itself (separate from, and in addition to, the `middleware.ts` redirect-to-`/login` for the route). Renders boutiques, recent orders (French status labels for `commandes.statut`), and mounts `NouvelleVente`.
- **`app/dashboard/NouvelleVente.tsx`** — vendor-facing "record a sale" POS form, calls `creer_vente` client-side. Supports both a real catalogue product line and a free-form "article libre" line (no `produit_id`) — see the `creer_vente` RPC description below for the exact validation split between the two shapes.
...
- This file's "What this repo is" and "Route protection" sections originally said there was no catalogue/cart/dashboard UI and zero business-table/RPC calls anywhere in the repo — stale as of the catalogue, cart, and vendor dashboard work (`app/catalogue`, `app/produit/[id]`, `app/panier`, `app/dashboard`); corrected above under "Consumer catalogue/cart UI" and "Vendor dashboard UI".
