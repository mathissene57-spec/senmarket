...
**RLS / ownership model**: almost every policy follows `vendor_id = auth.uid()` (via `boutiques`) or `boutique_id IN (SELECT id FROM boutiques WHERE vendor_id = auth.uid())`. Public read policies gate on `actif = true` (`produits`) — catalogue read policy additionally requires the owning boutique to be `actif = true` too (a product isn't publicly visible if its shop is off). As of Phase 3A, vendor `UPDATE` on `boutiques`/`produits` is restricted **by column** (`REVOKE`/`GRANT UPDATE (...)`), not just by row — a vendor can no longer set `boutiques.verifie`, `boutiques.note`, `boutiques.nb_produits`, `boutiques.nb_ventes`, `boutiques.nb_abonnes`, or `produits.ventes`/`ordre_accueil` directly.
