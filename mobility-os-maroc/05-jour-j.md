...
- [ ] `presentation-partenaire.pptx` ouvert une fois pour vérifier qu'il s'affiche correctement sur la machine utilisée samedi (polices de secours si Cambria/Calibri ne sont pas installées — le contenu reste lisible).
...
- [ ] Le PPTX intégrant déjà les captures d'écran des maquettes (`04-dossier-commercial/presentation-partenaire.pptx`), il sert de **filet de sécurité automatique** si la démo live ou le wifi pose problème — pas besoin de préparer autre chose pour ce cas.
